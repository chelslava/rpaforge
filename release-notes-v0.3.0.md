...
**Before:**
```python
# Thread-based timeout (could leak resources)
result = activity.run(timeout_seconds=30)
```

**After:**
```python
# Subprocess-based timeout (isolated, no leaks)
result = activity.run(timeout_seconds=30)  # Runs in subprocess
```
...
condition = eval(user_input)  # DANGEROUS
```

**After:**
```python
# Safe - AST-based parsing with limited operators
from rpaforge.core.safe_evaluator import safe_eval
condition = safe_eval(user_input, allowed_ops={'+', '-', '*', '/', '==', '!=', '<', '<=', '>', '>='})
```
...
No code changes required for existing projects. All improvements are transparent improvements:
...
This release was made possible by the community. Special thanks to all contributors and testers.
