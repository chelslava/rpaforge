...
class Excel:
    ...
    def _openpyxl(self):
        try:
            ...
            raise ImportError(
                _(
                    "openpyxl is required for Excel library. Install it with: pip install rpaforge-libraries[excel]"
                )
            ) from err
    ...
    def read_range(
        self, range_spec: str, sheet: str | None = None, as_dict: bool = False
    ...
    @tags("range", "write")
    def write_range(
        self, start_cell: str, data: list[list[Any]], sheet: str | None = None
    ) -> None:
        if not self._workbook:
            raise ValueError(_("No workbook open"))
        ws = self._workbook[sheet] if sheet else self._workbook.active
    ...
    def read_sheet_to_list(
        self, sheet: str | None = None, header_row: int = 1, max_rows: int | None = None
    ) -> list[dict[str, Any]]:
