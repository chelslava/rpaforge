...
def _ensure_screen_capture() -> None:
    if sys.platform not in ('win32', 'darwin'):
        raise NotImplementedError(_('Screen capture with PIL ImageGrab requires Windows or macOS. ') + _('On Linux, consider using mss library instead: pip install mss'))
...
class OCR:
    ...
    def _tesseract(self):
        try:
            ...
            raise ImportError(_('pytesseract is required for OCR library. Install it with: pip install rpaforge-libraries[ocr]')) from err
    ...
    def _pillow(self):
        try:
            ...
            raise ImportError(_('pillow is required for OCR library. Install it with: pip install rpaforge-libraries[ocr]')) from err
    ...
    @output('Recognized text')
    def ocr_text_from_screen(self, region: tuple[int, int, int, int] | None=None) -> str:
    ...
    def find_text_on_screen(self, text: str, region: tuple[int, int, int, int] | None=None) -> tuple[int, int] | None:
        ...
        for i, word in enumerate(data['text']):
            if text.lower() in word.lower() and data['conf'][i] >= self._min_confidence * 100:
    ...
    @tags('ocr', 'click', 'text')
    def click_text(self, text: str, region: tuple[int, int, int, int] | None=None, button: str='left') -> bool:
    ...
    def get_text_coordinates(self, text: str, region: tuple[int, int, int, int] | None=None) -> dict[str, int] | None:
    ...
    def get_ocr_data(self, region: tuple[int, int, int, int] | None=None) -> list[dict[str, Any]]:
        ...
        for i in range(len(data['text'])):
            if data['text'][i].strip() and data['conf'][i] > 0:
                item = {'text': data['text'][i], 'x': data['left'][i], 'y': data['top'][i], 'width': data['width'][i], 'height': data['height'][i], 'confidence': data['conf'][i] / 100.0}
                if region:
                    item['x'] += region[0]
                    item['y'] += region[1]
                results.append(item)
    ...
    def _get_ocr_data(self, region: tuple | None=None) -> dict:
        ...
        if region:
            ...
            image = ImageGrab.grab()
        try:
            return pytesseract.image_to_data(image, lang=self._lang, output_type=pytesseract.Output.DICT)
        finally:
            image.close()
    ...
    def ocr_with_confidence(self, path: str, lang: str | None=None, min_confidence: float | None=None) -> list[dict[str, Any]]:
        ...
        with Image.open(path) as image:
            data = pytesseract.image_to_data(image, lang=lang or self._lang, output_type=pytesseract.Output.DICT)
        results = []
        for i, word in enumerate(data['text']):
        ...
        logger.info(_('ocr_with_confidence_words_above', count=len(results), threshold=threshold))
        return results
    ...
    def read_barcode(self, path: str) -> list[str]:
        ...
        try:
            ...
            raise ImportError(_('pyzbar is required for barcode reading. Install with: pip install pyzbar  (libzbar-0 also required on Linux)')) from err
        Image, _ = self._pillow
        with Image.open(path) as image:
