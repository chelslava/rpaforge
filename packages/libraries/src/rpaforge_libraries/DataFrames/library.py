...
class DataFrames:
    ...
    def _pl(self):
        try:
            ...
            raise ImportError(_('polars is required for DataFrames library. Install it with: pip install rpaforge-libraries[dataframes]')) from err

    def _get_frame(self, name: str) -> Any:
        if name not in self._frames:
            raise KeyError(_("DataFrame '{name}' not found. Available: {available}", name=name, available=list(self._frames.keys())))
    ...
    def read_csv(self, path: str, frame_name: str='df', separator: str=',', has_header: bool=True) -> str:
        ...
        logger.info(f"Read CSV '{path}': {df.shape[0]}r x {df.shape[1]}c → '{frame_name}'")
        return frame_name

    @activity(name='Read Excel', category='DataFrames')
    @tags('excel', 'read', 'file')
    @output('Name of the loaded DataFrame')
    def read_excel(self, path: str, frame_name: str='df', sheet: str | None=None) -> str:
        ...
        self._frames[frame_name] = df
        logger.info(f"Read Excel '{path}': {df.shape[0]}r x {df.shape[1]}c → '{frame_name}'")
        return frame_name

    @activity(name='Read JSON', category='DataFrames')
    @tags('json', 'read', 'file')
    @output('Name of the loaded DataFrame')
    def read_json(self, path: str, frame_name: str='df') -> str:
        ...
        self._frames[frame_name] = df
        logger.info(f"Read JSON '{path}': {df.shape[0]}r x {df.shape[1]}c → '{frame_name}'")
        return frame_name

    @activity(name='From List', category='DataFrames')
    @tags('create', 'list')
    @output('Name of the created DataFrame')
    def from_list(self, data: list, frame_name: str='df') -> str:
        """Create a DataFrame from a list of dicts."""
        pl = self._pl
        df = pl.DataFrame(data)
        self._frames[frame_name] = df
        logger.info(f"Created DataFrame from list: {df.shape[0]}r x {df.shape[1]}c → '{frame_name}'")
        return frame_name
    ...
    @param('separator', type='string', description='Column separator character')
    def write_csv(self, frame: str, path: str, separator: str=',') -> str:
    ...
    @output('Path where the file was saved')
    def write_excel(self, frame: str, path: str, sheet: str='Sheet1') -> str:
    ...
    @output('Path where the file was saved')
    def write_json(self, frame: str, path: str) -> str:
    ...
    @param('operator', type='string', description='Comparison operator', options=['==', '!=', '>', '>=', '<', '<=', 'contains', 'starts_with', 'ends_with', 'is_null', 'is_not_null'])
    def filter_rows(self, frame: str, column: str, operator: str, value: str='', result_frame: str | None=None) -> str:
        ...
        if operator in {'>', '>=', '<', '<='}:
            try:
                ...
                numeric_value = value
            ops = {'>': col.__gt__, '>=': col.__ge__, '<': col.__lt__, '<=': col.__le__}
            ...
            raise ValueError(_("Unknown operator '{operator}'. Use one of: {operators}", operator=operator, operators=', '.join(['==', '!=', '>', '>=', '<', '<=', 'contains', 'starts_with', 'ends_with', 'is_null', 'is_not_null'])))
        result = df.filter(mask)
        target = self._save_frame(result_frame, frame, result)
        logger.info(f"Filter '{frame}': {column} {operator} {value!r} → {result.shape[0]} rows → '{target}'")
        return target

    @activity(name='Select Columns', category='DataFrames')
    @tags('select', 'columns')
    @output('Name of the resulting DataFrame')
    def select_columns(self, frame: str, columns: list, result_frame: str | None=None) -> str:
    ...
    @tags('drop', 'columns')
    @output('Name of the resulting DataFrame')
    def drop_columns(self, frame: str, columns: list, result_frame: str | None=None) -> str:
    ...
    @tags('slice', 'rows')
    @output('Name of the resulting DataFrame')
    def slice_rows(self, frame: str, start: int, length: int | None=None, result_frame: str | None=None) -> str:
    ...
    @tags('sort', 'order')
    @output('Name of the resulting DataFrame')
    def sort(self, frame: str, by: list, descending: bool=False, result_frame: str | None=None) -> str:
    ...
    @tags('rename', 'column')
    @output('Name of the resulting DataFrame')
    def rename_column(self, frame: str, old_name: str, new_name: str, result_frame: str | None=None) -> str:
    ...
    @tags('null', 'clean')
    @output('Name of the resulting DataFrame')
    def drop_nulls(self, frame: str, subset: list | None=None, result_frame: str | None=None) -> str:
    ...
    @tags('null', 'fill')
    @output('Name of the resulting DataFrame')
    def fill_nulls(self, frame: str, value: Any, column: str | None=None, result_frame: str | None=None) -> str:
        ...
        target = self._save_frame(result_frame, frame, result)
        logger.info(f"Filled nulls in '{frame}' column={column} with {value!r} → '{target}'")
        return target
    ...
    @param('function', type='string', description='Aggregation function', options=['sum', 'mean', 'min', 'max', 'count', 'std', 'first', 'last'])
    def aggregate(self, frame: str, column: str, function: str='sum') -> Any:
        """Compute a single aggregation over a column."""
        pl = self._pl
        df = self._get_frame(frame)
        col = pl.col(column)
        agg_map = {'sum': col.sum(), 'mean': col.mean(), 'min': col.min(), 'max': col.max(), 'count': col.count(), 'std': col.std(), 'first': col.first(), 'last': col.last()}
        if function not in agg_map:
            raise ValueError(_("Unknown function '{function}'. Use: {functions}", function=function, functions=', '.join(self._AGG_FUNCTIONS)))
    ...
    @output('Name of the resulting DataFrame')
    @param('agg_function', type='string', description='Aggregation function', options=['sum', 'mean', 'min', 'max', 'count', 'std', 'first', 'last'])
    def group_by(self, frame: str, by: list, agg_column: str, agg_function: str='sum', result_frame: str | None=None) -> str:
        ...
        if agg_function not in agg_map:
            raise ValueError(_("Unknown function '{agg_function}'. Use: {functions}", agg_function=agg_function, functions=', '.join(self._AGG_FUNCTIONS)))
        result = df.group_by(by).agg(agg_map[agg_function])
        target = self._save_frame(result_frame, frame, result)
        logger.info(f"group_by {by}, {agg_function}('{agg_column}') on '{frame}' → '{target}'")
        return target

    @activity(name='Join', category='DataFrames')
    @tags('join', 'merge')
    @output('Name of the resulting DataFrame')
    @param('how', type='string', description='Join type', options=['inner', 'left', 'full', 'cross', 'semi', 'anti'])
    def join(self, left_frame: str, right_frame: str, on: list, how: str='inner', result_frame: str | None=None) -> str:
        ...
        logger.info(f"Joined '{left_frame}' + '{right_frame}' on {on} ({how}) → '{target}'")
        return target

    @activity(name='Concat', category='DataFrames')
    @tags('concat', 'append')
    @output('Name of the resulting DataFrame')
    def concat(self, frames: list, result_frame: str | None=None) -> str:
