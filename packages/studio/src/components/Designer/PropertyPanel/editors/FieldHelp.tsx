  ...
  return (
    <div className="relative inline-flex items-center">
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
className="ml-1 text-slate-400 dark:text-slate-500 hover:text-indigo-500 transition-colors"
...
<button onClick={() => setOpen(false)} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"><FiX className="w-3 h-3" /></button>
          </div>
          {description && <p className="text-slate-600 dark:text-slate-300 mb-2">{description}</p>}
          {format && (
            <div className="mb-2">
<span className="text-slate-500 dark:text-slate-400 font-medium">{t('fieldHelp.format')} </span>
              <code className="bg-slate-100 dark:bg-slate-700 px-1 rounded">{format}</code>
            </div>
          )}
          {examples && examples.length > 0 && (
            <div>
<span className="text-slate-500 dark:text-slate-400 font-medium block mb-1">{t('fieldHelp.examples')}</span>
              {examples.map(ex => (
