...
function WithBreakpointComponent({ nodeId, children }: WithBreakpointProps) {
  ...
  const isPinned = useBlockStore(
    (state) => state.nodes.find((n) => n.id === nodeId)?.data?.pinned ?? false
  );
  ...
  return (
    <div className="relative">
      ...
      {isPinned && (
        <div
          className="absolute -right-1 -top-1 w-4 h-4 bg-ui-primary rounded-full border-2 border-ui-surface shadow-sm z-10 flex items-center justify-center pointer-events-none"
          title={t('canvas.pinnedPosition')}
          aria-label={t('canvas.pinnedPosition')}
        >
          <FiMapPin size={8} className="text-white" />
        </div>
      )}
