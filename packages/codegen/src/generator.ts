// @ts-nocheck
/**
 * Python code generator from RPA diagrams
 * Framework-agnostic: uses RpaNode/RpaEdge from @rpaforge/domain-model
 */

import type { RpaNode, RpaEdge } from '@rpaforge/domain-model';
import { buildGraph, findCommonMergeNode } from './graph';
import { getActivityKeyword, formatSwitchCondition, reprValue, sanitizeIdentifier, sanitizeString } from './utils';

interface CodegenDiagram<D = any> {
  nodes: RpaNode<D>[];
  edges: RpaEdge[];
}
...
export function findStartNode<D = any>(nodes: RpaNode<D>[]): RpaNode<D> | null {
  return nodes.find((node) => node.data?.blockData?.type === 'start') || null;
}
...
export function generatePythonCode(diagram: CodegenDiagram): string {
  ...
  const generateNode = (nodeId: string | undefined, indent = 1, stopNode?: string): string[] => {
    ...
    const blockData = (node?.data as any)?.blockData;
    if (!node || !blockData) {
      return [];
    }
