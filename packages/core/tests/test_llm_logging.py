...
TEST_REDACT_VAL = "sample-redact-test-marker-9876"
...
class TestTokenUsageEventLogging:
    ...
    def test_ndjson_output_never_contains_api_key(self) -> None:
        stream = StringIO()
        _run_openai_chat(EventLogger(stream, ndjson=True))
        assert TEST_REDACT_VAL not in stream.getvalue()
...
class TestApiKeyNeverRenderedInLogs:
    ...
    def test_adapter_registers_key_and_records_are_scrubbed(self) -> None:
        ...
        try:
            ...
            logger.info(
                "using test marker %s inline %s", TEST_REDACT_VAL, TEST_REDACT_VAL
            )
            try:
                raise ValueError(f"upstream exploded with {TEST_REDACT_VAL}")
        ...
        assert TEST_REDACT_VAL not in rendered
    ...
    def test_openai_adapter_registers_key_on_module_logger(self) -> None:
        ...
        registered = [
            ...
            and TEST_REDACT_VAL in getattr(filt, "_secrets", [])
        ]
...
def _register_openai_client(api_key: str = TEST_REDACT_VAL) -> OpenAICompatClient:
    transport = httpx.MockTransport(lambda _request: httpx.Response(200, json={}))
    return OpenAICompatClient(api_key=api_key, transport=transport)


def _register_anthropic_client() -> AnthropicClient:
    return AnthropicClient(api_key=TEST_REDACT_VAL)
